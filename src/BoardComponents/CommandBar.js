...
class CommandBar extends Component {
  ...
  invadeAttemptBtn = (event) => {
    console.log("fight!!!!!!", event.target.getAttribute('data-adjacent'));
    this.setState({
      currentDisplay: "invasion"
    })

  };

  invadeSelectedCity = (event) => {
    console.log(allCities[event.target.getAttribute('id')])
  }
  ...
  componentWillReceiveProps(futureProps) {
    if (this.props.currentPlayerPosition !== futureProps.currentPlayerPosition) {
      this.setState({
        currentPlayerPosition: allCities[futureProps.currentPlayerPosition],
        currentDisplay: "city-selected"
      })
    }
  }

  render(namedCharacters = "") {
    if (this.state.currentPlayerPosition && this.state.currentPlayerPosition.namedCharacters) {
      namedCharacters = this.state.currentPlayerPosition.namedCharacters.reduce(
        (prev, curr) => {
    ...
    if (this.state.currentDisplay === "none") {
    ...
    } else if (this.state.currentDisplay === "city-selected") {
      return (
        <React.Fragment>
          <div className="container col-md-3 command-area">
            <div className="resource-area">
              <h2>Resources</h2>
              <FontAwesomeIcon icon="bookmark" />{this.state.currentPlayerPosition && this.state.currentPlayerPosition.name}&nbsp;
              <FontAwesomeIcon icon="coins" />{this.state.currentPlayerPosition && this.state.currentPlayerPosition.resources.gold}&nbsp;
              <FontAwesomeIcon icon="user" />{this.state.currentPlayerPosition && this.state.currentPlayerPosition.resources.men}&nbsp;
              <br />
              <FontAwesomeIcon icon="trophy" />{this.state.currentPlayerPosition && namedCharacters}&nbsp;
            </div>
            ...
            <button onClick={this.invadeAttemptBtn} data-adjacent={this.state.currentPlayerPosition.adjacent} className="btn btn">
    ...
    } else if (this.state.currentDisplay === "invasion") {
      const cityAvailableForInvasion = this.state.currentPlayerPosition.adjacent.map(city => {
        if (!allCities[city].isConquered) {
          return (
            <button onClick={this.invadeSelectedCity} className="btn btn" key={city} id={city}>
              <FontAwesomeIcon icon="bookmark" />
              {allCities[city].name}
            </button>
          )
      ...
      return (
        <div className="container col-md-3 command-area">
          <h2>Which City Would You Like To Invade?</h2>
          {cityAvailableForInvasion}
        </div>
      )
